...
from pyspark.sql.functions import regexp_replace, col, regexp_extract, max as spark_max
...
files_df = files_df.withColumn("date", regexp_extract(col("path"), r"(\d{4}\d{2}\d{2})", 1))
files_df = files_df.withColumn("new_path", regexp_replace(col("path"), r"(\d{4}\d{2}\d{2}).*", r".csv")).drop("path")
files_df=files_df.groupBy("new_path").agg(spark_max("date")).alias("latest_date_file")
# # Calculate max date and max modificationTime for the same date
# max_date_df = files_df.groupBy("date").agg(
#     spark_max("modificationTime").alias("max_modificationTime")
# ).orderBy(col("date").desc()).limit(1)
