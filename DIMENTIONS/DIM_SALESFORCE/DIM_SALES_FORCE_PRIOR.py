...
tables_list = ["IST_DIM_SALES_FORCE_PRIOR"]
...
df = spark.sql("select * from db_sil_temp_delta.IST_DIM_SALES_FORCE_PRIOR")
...
df.write.option("compression", "snappy").option("overwriteSchema", "true").option("mergeSchema", "true").mode("overwrite").format("parquet").saveAsTable("db_sil_temp_delta.DIM_SALES_FORCE_PRIOR")

# COMMAND ----------

# MAGIC %sql
# MAGIC refresh table db_sil_temp_delta.DIM_SALES_FORCE_PRIOR
